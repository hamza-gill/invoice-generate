...
@section('title', 'Edit Invoice - ' . ($globalSettings->company_name ?? config('app.name')))
@php($hideNavbar = true)
...
@section('content')
    <div class="bg-gray-50">
        ...
        <header class="bg-white border-b border-gray-200 px-8 py-4 flex items-center justify-between shadow-sm sticky top-0 z-20">
            <div class="flex items-center space-x-4">
                <a href="{{ route('invoices.show', $invoice->id) }}" class="text-gray-600 hover:text-gray-800">
                    <i class="fas fa-arrow-left"></i>
                </a>
                <h2 class="text-xl font-bold text-gray-800">Edit Invoice #{{ $invoice->invoice_number }}</h2>
            </div>
        </header>
        ...
        <main class="p-8">
            <div class="max-w-5xl mx-auto space-y-8">
                ...
                {{-- FORM --}}
                <form id="invoiceForm" action="{{ route('invoices.update', $invoice->id) }}" method="POST" class="space-y-8">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <label class="cursor-pointer group">
                                    <input type="radio" name="invoice_template_id" value="" class="hidden peer template-radio" {{ old('invoice_template_id', $invoice->invoice_template_id) ? '' : 'checked' }}>
                                ...
                                @foreach($templates as $tpl)
                                    <label class="cursor-pointer group">
                                        <input type="radio" name="invoice_template_id" value="{{ $tpl->id }}" class="hidden peer template-radio" {{ (int) old('invoice_template_id', $invoice->invoice_template_id) === $tpl->id ? 'checked' : '' }}>
                                        <div class="relative border-2 border-gray-200 peer-checked:border-blue-600 peer-checked:ring-2 peer-checked:ring-blue-100 rounded-xl p-4 text-center transition-all group-hover:border-blue-300 group-hover:shadow-md">
                    ...
                    <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <button type="button" id="toggleCustomerFields" class="text-blue-600 hover:text-blue-800 font-medium text-sm">
                                <i class="fas fa-pen mr-1"></i> Edit Customer Details
                            </button>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-6 mb-6">
                            <div class="col-span-2">
                                ...
                                <p class="text-xs text-gray-500 mt-2">
                                    Search for a different customer, choose <span class="font-medium">"+ Add New Customer"</span>, or use "Edit Customer Details" above to correct this customer's info.
                                </p>
                            </div>
                        </div>
                        ...
                        <div id="customerDetailsFields" class="grid grid-cols-2 gap-6 hidden">
                            <div>
                                ...
                                <input type="text" id="customer_first_name" name="first_name" value="{{ old('first_name', $invoice->customer->first_name ?? '') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600" readonly>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Last Name *</label>
                                <input type="text" id="customer_last_name" name="last_name" value="{{ old('last_name', $invoice->customer->last_name ?? '') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600" readonly>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Company Name</label>
                                <input type="text" id="customer_company_name" name="company_name" value="{{ old('company_name', $invoice->customer->company_name ?? '') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600" readonly>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Email *</label>
                                <input type="email" id="customer_email" name="email" value="{{ old('email', $invoice->customer->email ?? '') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600" readonly>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Phone Number</label>
                                <input type="text" id="customer_phone_number" name="phone_number" value="{{ old('phone_number', $invoice->customer->phone_number ?? '') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600" readonly>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Address *</label>
                                <input type="text" id="customer_address" name="address" value="{{ old('address', $invoice->customer->address ?? '') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600" readonly>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">City *</label>
                                <input type="text" id="customer_city" name="city" value="{{ old('city', $invoice->customer->city ?? '') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600" readonly>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">State *</label>
                                <input type="text" id="customer_state" name="state" value="{{ old('state', $invoice->customer->state ?? '') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600" readonly>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Zip Code *</label>
                                <input type="text" id="customer_postal_code" name="postal_code" value="{{ old('postal_code', $invoice->customer->postal_code ?? '') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600" readonly>
                            </div>
                            <div>
                                ...
                                <select id="customer_country" name="country" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 bg-white">
                                    ...
                                    @include('customers.partials.countries', ['selected' => old('country', $invoice->customer->country ?? null)])
                                </select>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="flex items-start space-x-4">
                                <div class="flex items-center h-5">
                                    <input type="checkbox"
                                           ...
                                           value="1"
                                           {{ old('enable_rush_delivery', $invoice->enable_rush_addon) ? 'checked' : '' }}
                        ...
                        <div class="mt-6 pt-6 border-t border-gray-200">
                            <div class="flex justify-end mb-4">
                                <div class="w-64 text-md text-gray-800">
                                    ...
                                    <input type="number" step="0.01" id="discount" name="discount" value="{{ old('discount', $invoice->discount ?? 0) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-right">
                                </div>
                            </div>
                    ...
                    <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                        ...
                        <textarea name="notes"
                                  id="notes"
                                  rows="4"
                                  class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600">{{ old('notes', $invoice->note ?? '') }}</textarea>
                    </div>
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400">Shows your unsaved changes — nothing has been updated yet</p>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            const NEW_CUSTOMER_OPTION = { id: 'new', text: '+ Add New Customer' };
            ...
            // Manual toggle — lets the user correct the CURRENTLY selected customer's
            // details without having to pick "+ Add New Customer".
            $('#toggleCustomerFields').on('click', function () {
            ...
            $('#customerSelect').on('select2:select', function(e){
                ...
                // A different existing customer was picked — values are set, hide the fields again
                hideCustomerFields();
            });
            ...
            function addLineItem(preset = null){
                ...
                row.innerHTML = `
                    ...
                    <div class="col-span-2">
                        <input type="number" name="line_items[${index}][unit_price]" step="0.01" class="line-price w-full px-3 py-2 border border-gray-300 rounded-lg text-right" value="${preset ? preset.unitPrice : 0}" required>
                        <input type="hidden" class="line-quantity" value="${preset ? preset.quantity : 1}">
                    </div>
                    <div class="col-span-2">
                    ...
                    <input type="hidden" name="line_items[${index}][product_id]" class="line-product-id" value="${preset ? preset.productId : ''}">
                    <input type="hidden" name="line_items[${index}][quantity]" value="${preset ? preset.quantity : 1}">
                    <input type="hidden" name="line_items[${index}][description]" class="line-description" value="${preset ? preset.description : ''}">
                `;
                ...
                // If preloading an existing line item, seed select2 with its current
                // product as an already-selected local option before wiring up ajax.
                if (preset && preset.productId) {
                    const opt = new Option(preset.productName, preset.productId, true, true);
            ...
            function loadInvoicePreview() {
                ...
                // Strip the PUT _method field so the form posts as a plain POST
                // to the preview route (which is defined as POST only).
                const serialized = $(form).serializeArray().filter(function (f) {
