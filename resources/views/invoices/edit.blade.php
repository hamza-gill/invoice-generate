    ...
    <div class="min-h-screen bg-gray-50 flex flex-col">
        ...
        <header class="bg-white border-b border-gray-200 px-8 py-4 flex items-center justify-between shadow-sm sticky top-0 z-20">
            <div class="flex items-center space-x-4">
                ...
                <h2 class="text-xl font-bold text-gray-800">Edit Invoice</h2>
            </div>
        </header>
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text"
                                       ...
                                       name="company_name"
                                       value="{{ old('company_name', $invoice->customer->company_name ?? '') }}"
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text"
                                       id="customer_address"
                                       name="address"
                                       value="{{ old('address', $invoice->customer->address ?? '') }}"
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600"
                                       required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text"
                                       id="customer_city"
                                       name="city"
                                       value="{{ old('city', $invoice->customer->city ?? '') }}"
                                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600"
                                       required>
                            </div>
                        </div>
                            ...
                            @foreach($invoice->items as $index => $item)
                                <div class="grid grid-cols-12 gap-4 items-center bg-gray-50 p-4 rounded-lg border border-gray-200">
                                    <div class="col-span-8">
                                        <input type="text" value="{{ $item->product->name }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 bg-gray-100" readonly>
                                    </div>
                                    <div class="col-span-3">
                                        <input type="text" class="line-total w-full px-3 py-2 bg-gray-100 border border-gray-300 rounded-lg text-right font-semibold text-gray-800"
                                               value="${{ number_format($item->quantity * $item->amount, 2) }}" readonly
                                               data-value="{{ $item->quantity * $item->amount }}">
                                    </div>
                                    ...
                                    <input type="hidden" name="line_items[{{ $index }}][quantity]" value="{{ $item->quantity }}">
                                    <input type="hidden" name="line_items[{{ $index }}][unit_price]" value="{{ $item->amount }}">
                                    <input type="hidden" name="line_items[{{ $index }}][description]" value="{{ $item->product->description }}">
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="mt-6 pt-6 border-t border-gray-200">
                            <div class="flex justify-end">
                                <div class="w-64 text-lg font-semibold text-gray-800 flex justify-between">
                                    <span>Total:</span>
                                    <span id="totalAmount">
                                        ${{ number_format($invoice->items->sum(fn($i) => $i->quantity * $i->amount), 2) }}
                                    </span>
                                </div>
                            </div>
                        </div>
